...
// Remove the placeholder dialog components since we're now importing the real ones
...
export default function Roles() {
    ...
    console.log('Roles page data:', pageData);

    const roles = Array.isArray(pageData.roles) ? pageData.roles : Array.isArray(pageData.roles?.data) ? pageData.roles.data : [];
    const pagination = (Array.isArray(pageData.roles) ? null : pageData.roles) || { current_page: 1, last_page: 1, per_page: 10, total: 0 };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Roles" />
            <div className="w-full">
                <RolesTable roles={roles} pagination={'meta' in pagination ? pagination.meta : pagination} />
            </div>
        </AppLayout>
    );
}
...
function RolesTable({
    ...
    const availableGuards = React.useMemo(() => {
        ...
        tableData.forEach(role => {
            if (role.guard_name) guards.add(role.guard_name);
        });
    ...
    useEffect(() => {
        try {
            setIsLoading(true);
            const processedData = (roles || []).map(role => ({
                id: String(role?.id || ''),
    ...
    useEffect(() => {
        ...
        // Guard filter
        if (guardFilter) {
            result = result.filter(role => role.guard_name === guardFilter);
        }
    ...
    const handleView = (id: string) => {
        try {
            const role = tableData.find(role => role.id === id);
            if (role) {
    ...
    if (tableData.length === 0 && !isLoading) {
        return (
            <div className="p-4 border rounded">
                <p>No roles found or error loading roles.</p>
